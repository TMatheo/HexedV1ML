...
namespace LUXED.UIApi
{
    ...
    {
        public MMSingleButton(Transform btnMenu, float btnXLocation, float btnYLocation, string btnText, Action btnAction, string btnToolTip, MMButtonType Size = MMButtonType.Big, Sprite Sprite = null)
        {
            parentPage = btnMenu;
            InitButton(btnXLocation, btnYLocation, btnText, btnAction, btnToolTip, Size, Sprite);
        }
        private void InitButton(float btnXLocation, float btnYLocation, string btnText, Action btnAction, string btnToolTip, MMButtonType Size = MMButtonType.Big, Sprite Sprite = null)
        {
            switch (Size)
            {
                case MMButtonType.Big:
                    baseObject = UnityEngine.Object.Instantiate(MenuHelper.mainBigSingleButtonTemplate, parentPage);
                    break;
                case MMButtonType.Medium:
                    baseObject = UnityEngine.Object.Instantiate(MenuHelper.mainMediumSingleButtonTemplate, parentPage);
                    break;
                case MMButtonType.Small:
                    baseObject = UnityEngine.Object.Instantiate(MenuHelper.mainSmallSingleButtonTemplate, parentPage);
                    break;
            }
            SetActive(false);
            baseObject.name = $"Hexed_Button_{EncryptUtils.RandomString(10)} {EncryptUtils.RandomStringNumber(10)}";
    ...
    public class QMMenuPage
    {
        ...
        private void Initialize(string menuTitle)
        {
            ...
            MenuObject.name = $"Hexed_Menu_{EncryptUtils.RandomString(10)}-{EncryptUtils.RandomStringNumber(10)}";
        ...
        public void OpenMe()
        {
            ...
            MenuHelper.menuStateController.Method_Public_Void_String_UIContext_Boolean_EnumNPublicSealedvaNoLeRiBoIn6vUnique_0(MenuPage.field_Public_String_0, null, false, (UIPage.EnumNPublicSealedvaNoLeRiBoIn6vUnique)2);
        }
    ...
    public class QMSlider
    {
        ...
        public QMSlider(QMMenuPage btnMenu,float uppos, string text,string tooltip, Action<float, QMSlider> listener, float minValue, float maxValue, float defaultValue, bool shouldresetbtn)
        {
            parentPage = btnMenu.GetMenuObject().transform;
            InitSlider(uppos,text,tooltip,listener,minValue,maxValue,defaultValue,shouldresetbtn);
        }
        private void InitSlider(float btnYLocation,string text,string tooltip, Action<float, QMSlider> listener, float minValue, float maxValue, float defaultValue,bool shouldresetbtn = false)
        {
            gameObject = UnityEngine.Object.Instantiate(MenuHelper.sliderButtonTemplate, parentPage);
            gameObject.name = $"Hexed_QMSlider_{EncryptUtils.RandomString(10)}{EncryptUtils.RandomStringNumber(10)}";
            (TextMeshPro = gameObject.transform.Find("LeftItemContainer/Title").GetComponent<TextMeshProUGUI>()).text = text;
            TextMeshPro.richText = true;
            LocalizableString text2 = LocalizableStringExtensions.Localize(tooltip);
            (slider = gameObject.transform.Find("RightItemContainer/Slider")).GetComponent<ToolTip>()._localizableString = text2;
            (snapSlider = slider.GetComponent<SnapSliderExtendedCallbacks>()).field_Private_UnityEvent_0 = null;
            gameObject.transform.localPosition = new Vector3(-461f,btnYLocation,0);
            ...
            snapSlider.onValueChanged.AddListener(new Action<float>((va) => {
                listener.Invoke(va, this);
            }));
            if (shouldresetbtn)
            {
                var Btn = slider.parent.Find("Button");
                LocalizableString text23 = LocalizableStringExtensions.Localize("Reset slider", null, null, null);
                Btn.GetComponent<ToolTip>()._localizableString = text23;
                (Btn.GetComponent<Button>().onClick = new UnityEngine.UI.Button.ButtonClickedEvent()).AddListener(new Action(() => { snapSlider.value = snapSlider.minValue; }));
    ...
    public class QMIconButton : QMButtonBase
    {
        ...
        private void InitButton(float btnXLocation, float btnYLocation, Action btnAction, string btnToolTip, Sprite Sprite = null)
        {
            ...
            baseObject.name = $"Hexed-IconButton-{EncryptUtils.RandomString(10)}-{EncryptUtils.RandomStringNumber(10)}";
    ...
    public class QMScrollButton : QMButtonBase
    {
        ...
        public List<ScrollObject> ScrollButtons = new List<ScrollObject>();
        ...
        public void Add(string Text, string Description, Action Action)
        {
            ...
            QMSingleButton Button = new QMSingleButton(Page, Posx, Posy, Text, Action, Description, ButtonAPI.ButtonSize.Half);
