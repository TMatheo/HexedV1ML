...
namespace LUXED.Wrappers
{
    internal static class UnityUtils
    {
        ...
        private static readonly Dictionary<Sprite, Sprite> resourceSpritesOverride = new Dictionary<Sprite, Sprite>();
        ...
        {
            Stream stream = Assembly.GetExecutingAssembly().GetManifestResourceStream($"LUXED.Resources.{resourceName}.png");
            if (stream == null)
            {
                HDLogger.LogError($"Failed to find texture {resourceName}");
                return null;
            }
        ...
        public readonly struct InfPosition
        {
            ...
            public static Vector3 INFBypass => new Vector3(InfValue, InfValue, InfValue);
            public static Vector3 NegINFBypass => new Vector3(-InfValue, -InfValue, -InfValue);
            public static Quaternion ROTBypass => new Quaternion(InfValue, InfValue, InfValue, InfValue);
            public static Quaternion NegROTBypass => new Quaternion(-InfValue, -InfValue, -InfValue, -InfValue);
        }

        private static readonly Dictionary<Texture2D, Texture2D> cachedReadableTexture2D = new Dictionary<Texture2D, Texture2D>();
        private static readonly Dictionary<Texture, Texture2D> cachedConvertedTexture2D = new Dictionary<Texture, Texture2D>();
        private static readonly Dictionary<Texture2D, Texture2D> cachedDesaturatedTexture2D = new Dictionary<Texture2D, Texture2D>();
        private static readonly Dictionary<Sprite, Texture2D> cachedUnpackedTexture2D = new Dictionary<Sprite, Texture2D>();
