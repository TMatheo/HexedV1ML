...
namespace LUXED.CustomUI.QM
{
    internal class UtilsMenu
    {
        ...
        {
            UtilsPage = new QMMenuPage("Utils");

            QMSingleButton OpenMenu = new QMSingleButton(MainMenu.ClientPage, 2, 0, "Utils", UtilsPage.OpenMe, "Utils Options", ButtonAPI.ButtonSize.Default, UnityUtils.GetSprite("Utils"));
            ...
            new QMSingleButton(UtilsPage, 3, 1, "Drop \nPortal", delegate
            {
                GameHelper.VRCUiPopupManager.AskInGameInput("World ID", "Ok", delegate (string text)
                {
                    ...
                    {
                        PhotonHelper.RaisePortalCreate(text, GameHelper.CurrentPlayer.transform.position + Vector3.forward * 1.505f, 0);
                    }));
                }, "wrld_");
