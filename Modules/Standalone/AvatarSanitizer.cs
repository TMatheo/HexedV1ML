        ...
        public static void SanitizeAvatarObject(GameObject gameObject, VRCPlayer player)
        {
            ...
            foreach (Transform t in gameObject.GetComponentsInChildren<Transform>(true))
            {
                ...
                if (float.IsNaN(pos.x) || float.IsNaN(pos.y) || float.IsNaN(pos.z) ||
                    Mathf.Abs(pos.x) > 10000f || Mathf.Abs(pos.y) > 10000f || Mathf.Abs(pos.z) > 10000f)
                {
                    HDLogger.Log($"[Sanitize] Invalid position on GameObject '{t.name}' ({pos}) in avatar of {player.DisplayName()}", HDLogger.LogsType.Protection);
                    VRConsole.Log($"[Sanitize] {player.DisplayName()} has invalid object position: {t.name} -> {pos}", VRConsole.LogsType.Avatar);
                }
            }

            if (RemovedComponents > 0)
            {
                HDLogger.Log($"Prevented {player.DisplayName()} from overflow with {RemovedComponents} Avatar components", HDLogger.LogsType.Protection);
                ...
                else
                {
                    ...
                    if (skinnedRenderer != null)
                    {
                        ...
                        for (int b = 0; b < bones.Length; b++)
                        {
                            ...
                            {
                                GameObject newGo = new GameObject($"hexed-zero-scale");
