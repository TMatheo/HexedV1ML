...
namespace LUXED.Modules.Standalone
{
    internal class AssetChanger
    {
        ...
        {
            HDLogger.Log("Found custom Hide Robot", HDLogger.LogsType.Info);
            ...
            if (prefab_path == null)
            {
                HDLogger.LogError("Failed to find prefab in Assetbundle");
                yield break;
            }
        ...
        public static IEnumerator LoadCustomMusic()
        {
            ...
            if (!audioLoader.isHttpError)
            {
                ...
                HDLogger.Log("Initialized custom Loading Music", HDLogger.LogsType.Info);
            }
        }
