...
namespace LUXED.Modules.EventManagement
{
    internal class ModerationHandler
    {
        ...
        public static List<int> MuteList = new List<int>();
        ...
        public static bool ReceivedModerationEvent(ref Dictionary<byte, object> Event33Data)
        {
            switch (Convert.ToByte(Event33Data[0]))
            {
                case 2: // Alert
                    switch (Event33Data[2].ToString())
                    {
                        case "You have been warned for your behavior. If you continue, you may be kicked out of the instance":
                            HDLogger.Log($"Server tried to Warn you", HDLogger.LogsType.Room);
                ...
                case 8: // RoomOwner MicOff
                    HDLogger.Log($"Server tried to Mic Off you", HDLogger.LogsType.Room);
                    VRConsole.Log($"Server --> Mic Off", VRConsole.LogsType.Moderation);
                    return false;

                case 9: // Mod Mic Gain change
                    HDLogger.Log($"Server tried to change your Mic Volume", HDLogger.LogsType.Room);
                ...
                case 10: // Friend Player, still used or is that even a receive?
                    HDLogger.Log($"Server tried to move you to a new World", HDLogger.LogsType.Room);
                ...
                case 12: // Mod Teleport User
                    HDLogger.Log($"Server tried to Teleport you", HDLogger.LogsType.Room);
                ...
                case 13: // Votekick
                    ...
                    HDLogger.Log($"Votekick started on {Target} [ID: {Event33Data[3]}]", HDLogger.LogsType.Room);
                ...
                case 25: // Votekick Response
                    HDLogger.Log($"Votekick response is {Event33Data[2]} [ID: {Event33Data[3]}]", HDLogger.LogsType.Room);
                ...
                case 21: // Reply Moderations (Block / Mute)     
                    ...
                    if (MuteCheck == true)
                    {
                        if (!MuteList.Contains((int)Event33Data[1]))
                        {
                            ...
                            if (PhotonPlayer != null)
                            {
                                ...
                                HDLogger.Log($"{SenderName} muted you", HDLogger.LogsType.Room);
                    ...
                    else if (MuteCheck == false)
                    {
                        if (MuteList.Contains((int)Event33Data[1]))
                        {
                            ...
                            {
                                string SenderName = PhotonPlayer.DisplayName() ?? "NO NAME";
                                HDLogger.Log($"{SenderName} unmuted you", HDLogger.LogsType.Room);
                    ...
                    if (BlockCheck == true)
                    {
                        if (!BlockList.Contains((int)Event33Data[1]))
                        {
                            ...
                            {
                                string SenderName = PhotonPlayer.DisplayName() ?? "NO NAME";
                                HDLogger.Log($"{SenderName} blocked you", HDLogger.LogsType.Room);
                    ...
                    else if (BlockCheck == false)
                    {
                        if (BlockList.Contains((int)Event33Data[1]))
                        {
                            ...
                            {
                                string SenderName = PhotonPlayer.DisplayName() ?? "NO NAME";
                                HDLogger.Log($"{SenderName} unblocked you", HDLogger.LogsType.Room);
                ...
                default:
                    HDLogger.LogWarning($"Not implemented Moderation Event received with code {Event33Data[0]}");
                    break;
            }
            return true;
        }
        ...
        public static bool RaisedModerationEvent(Dictionary<byte, object> Event33Data)
        {
            switch (Convert.ToByte(Event33Data[0]))
            {
                ...
                default:
                    HDLogger.LogWarning($"Not implemented Moderation Event send with code {Event33Data[0]}");
                    break;
            }
